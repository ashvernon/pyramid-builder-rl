...
def main() -> None:
    ...
    ap.add_argument("--ckpt", default="", help="Path to checkpoint")
    ...
    env = PyramidEnv(sim_cfg, render=True, seed=int(rl_cfg["seed"]))
    ...
    for ep in range(args.episodes):
        ...
        while not done:
            action = agent.act(obs, goal, eps=0.0)
            obs, goal, done, info = env.step(action)
